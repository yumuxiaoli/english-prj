...
<div>
		...
		>
			...
			<el-table-column prop="username" header-align="center" align="center" label="姓名" min-width="150" />	
			<el-table-column prop="nickname" header-align="center" align="center" label="昵称" min-width="150" sortable="true"/>	
			...
			<el-table-column prop="email" header-align="center" align="center" label="邮箱" min-width="150" />	
			<el-table-column prop="phone" header-align="center" align="center" label="电话" min-width="150" />	
...
const showEffect = () => { //封装显示操作, 包含数据显示功能: 加载数据列表、查询、分页显示功能
  ...
  const loadDataList = async () => { //加载数据列表
      ...
      const data = { // 每次加载数据发送的数据对象,对应分页数据和查询数据
        ...
        username: ifEmpty(dataForm.username),
  ...
  const searchHandle = () => { //查询
    df.value.validate(valid => { //先执行表单验证
       if (valid) {
          ...
          dataForm.username = ifEmpty(dataForm.username)//因为服务器端进行正则验证，不允许上传空字符串给后端，但是可以传null值，
  ...
  const deleteHandle = (id) => { // 删除事件
    ...
    if (ids.length == 0) {
      ...
      ElMessageBox.confirm(`确定要删除选中的记录？`, '提示', {
        ...
        await post('user/deleteUserByIds', { ids }).then((resp) => {
