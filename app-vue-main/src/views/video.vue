  ...
  <div>
		<el-form :inline="true" :model="dataForm" :rules="dataRule" ref="df">
			...
			<el-form-item prop="videoSize">
				<el-input
					v-model="dataForm.videoSize"
					placeholder="大小" size="default"
					class="input" clearable="clearable"
					@blur="searchHandle()" @keyup.enter.native="$event.target.blur()"
			    />
			</el-form-item>
		...
		>
			...
			<el-table-column prop="title" header-align="center" align="center" label="题目" min-width="150" sortable="true"/>	
			<el-table-column prop="courseId" header-align="center" align="center" label="课程" min-width="150"/>	
...
import Add from './video-add.vue';
import Update from './video-update.vue';
const showEffect = () => { //封装显示操作, 包含数据显示功能: 加载数据列表、查询、分页显示功能
  ...
  const loadDataList = async () => { //加载数据列表
      ...
      const data = { // 每次加载数据发送的数据对象,对应分页数据和查询数据
        ...
        videoSize: ifEmpty(dataForm.videoSize),
        orderType: dataForm.orderType,
        orderColumn: dataForm.orderColumn
      };
      await post('video/listVideoByPage', data).then((resp) => {
  ...
  const searchHandle = () => { //查询
    df.value.validate(valid => { //先执行表单验证
       if (valid) {
          ...
          dataForm.videoSize = ifEmpty(dataForm.videoSize)//因为服务器端进行正则验证，不允许上传空字符串给后端，但是可以传null值，
  ...
  const deleteHandle = (id) => { // 删除事件
    ...
    if (ids.length == 0) {
      ...
      ElMessageBox.confirm(`确定要删除选中的记录？`, '提示', {
        ...
        await post('video/deleteVideoByIds', { ids }).then((resp) => {
