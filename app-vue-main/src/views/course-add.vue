...
const operateEffect = () => { // 封装页面操作逻辑
  ...
  const init = async () => { //弹层初始化
    ...
    nextTick(() => {
      ...
      introduceEditor.value.clearData()
    })
    await get('teacher/listValidTeacher').then((resp) => { //查询有效数据
      handleResponse({resp},
        () => {
          teacherList.value = resp.data.dataList
        }
      )
    })
    await get('assort/listValidAssort').then((resp) => { //查询有效数据
      handleResponse({resp},
        () => {
          assortList.value = resp.data.dataList
        }
      )
    })
  }
  const dataFormSubmit =  () => { //新增提交
    df.value.validate( async (valid) => {
      if (valid) { //表单验证
        ...
        dataForm.introduce = introduceEditor.value.getData()
        if (dataForm.introduce == ''){
          ElMessage({ message: '详情不能为空!', type: 'warning', offset: 200, center: true })
          return
        }
        dataLoading.value = true //显示动态加载
        await post('course/insert', dataForm).then((resp) =>{
  ...
  return {
    init, visible, dataForm, dataFormSubmit, df, dataLoading, imageUpload, assortList,teacherList, detailsEditor, introduceEditor,
  }
}
const validatorRule = () => { //封装客户端数据验证规则
  ...
  dataRule.value = {
    ...
    image:[
      { required: true, message: "图片路径格式错误", trigger: ['blur'] },
...
const { init, visible, dataForm, dataFormSubmit, df, dataLoading, imageUpload, assortList, teacherList, detailsEditor,introduceEditor } = operateEffect()
