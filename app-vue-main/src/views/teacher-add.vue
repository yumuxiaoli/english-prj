<template>
    ...
    >
			<el-form-item label="编号:" prop="number" >
				<el-input v-model="dataForm.number"  size="default" style="width:100%" clearable />
			</el-form-item>
  ...
  const dataFormSubmit =  () => { //新增提交
    df.value.validate( async (valid) => {
      if (valid) { //表单验证
        ...
        await post('teacher/insert', dataForm).then((resp) =>{
...
const validatorRule = () => { //封装客户端数据验证规则
  ...
  dataRule.value = {
    number: [
      { required: true, message: "编号格式错误", trigger: "blur" },
