<template>
	<div>
		...
		>
			...
			<el-table-column prop="number" header-align="center" align="center" label="编号" min-width="150" sortable="true"/>	
			<el-table-column prop="name" header-align="center" align="center" label="姓名" min-width="150" sortable="true"/>	
...
import Add from './teacher-add.vue';
import Update from './teacher-update.vue';
const showEffect = () => { //封装显示操作, 包含数据显示功能: 加载数据列表、查询、分页显示功能
  ...
  const loadDataList = async () => { //加载数据列表
      ...
      const data = { // 每次加载数据发送的数据对象,对应分页数据和查询数据
        ...
        number: ifEmpty(dataForm.number),
  ...
  const searchHandle = () => { //查询
    df.value.validate(valid => { //先执行表单验证
       if (valid) {
          ...
          dataForm.number = ifEmpty(dataForm.number)//因为服务器端进行正则验证，不允许上传空字符串给后端，但是可以传null值，
  ...
  const deleteHandle = (id) => { // 删除事件
    ...
    if (ids.length == 0) {
      ...
      ElMessageBox.confirm(`确定要删除选中的记录？`, '提示', {
        ...
        await post('teacher/deleteTeacherByIds', { ids }).then((resp) => {
...
const validatorRule = () => { //封装客户端数据验证规则
  ...
  dataRule.value = {
      number: [
        { required: false, message: "编号格式错误", trigger: "blur" },
        { whitespace: true, message: "编号格式错误", trigger: "blur" }
      ],
      name: [
        { required: false, message: "姓名格式错误", trigger: "blur" },
