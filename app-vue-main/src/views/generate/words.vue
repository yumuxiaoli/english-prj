<template>
	<div v-if="isAuth(['ROOT', 'WORDS:SELECT'])">
		<el-form :inline="true" :model="dataForm" :rules="dataRule" ref="df">
			<el-form-item prop="words">
				<el-input
					v-model="dataForm.words"
					placeholder="单词" size="default"
			...
			<el-form-item>
				...
				<el-button
					...
					:disabled="!isAuth(['ROOT', 'WORDS:INSERT'])"
					@click="addHandle()"
				>
					新增
				</el-button>
				<el-button
					...
					:disabled="!isAuth(['ROOT', 'WORDS:DELETE'])"
					@click="deleteHandle()"
				>
					批量删除
				</el-button>
		...
		>
			...
			<el-table-column prop="words" header-align="center" align="center" label="单词" min-width="150" sortable="true"/>	
			<el-table-column prop="pron" header-align="center" align="center" label="读音" min-width="150" />	
			<el-table-column prop="explains" header-align="center" align="center" label="解释" min-width="150" />	
			<el-table-column prop="grades" header-align="center" align="center" label="等级" min-width="150" />	
			...
			<el-table-column header-align="center" align="center" width="150" label="操作">
				<template #default="scope">
					<el-button
						link type="primary" size="default" :disabled="!isAuth(['ROOT', 'WORDS:UPDATE'])" @click="updateHandle(scope.row.id)"
					>
						修改
					</el-button>
					<el-button
						link type="primary" size="default" :disabled="!isAuth(['ROOT', 'WORDS:DELETE'])" @click="deleteHandle(scope.row.id)"
					>
						删除
					</el-button>
				</template>
...
import Add from './words-add.vue';
import Update from './words-update.vue';
  ...
  const deleteHandle = (id) => { // 删除事件
    ...
    if (ids.length == 0) {
      ...
      ElMessageBox.confirm(`确定要删除选中的记录？`, '提示', {
        ...
        await post('words/deleteWordsByIds', { ids }).then((resp) => {
