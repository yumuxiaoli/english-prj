<template>
	<div>
		...
		>
			...
			<el-table-column prop="title" header-align="center" align="center" label="题目" min-width="150" />	
			<el-table-column prop="rightanswer" header-align="center" align="center" label="正确答案" min-width="150" />	
			<el-table-column prop="selecta" header-align="center" align="center" label="选项A" min-width="150" />	
			<el-table-column prop="selectc" header-align="center" align="center" label="选项C" min-width="150" />	
			<el-table-column prop="selectd" header-align="center" align="center" label="选项D" min-width="150" />	
...
import Add from './excise-add.vue';
import Update from './excise-update.vue';
  ...
  const searchHandle = () => { //查询
    df.value.validate(valid => { //先执行表单验证
       if (valid) {
          ...
          dataForm.title = ifEmpty(dataForm.title)//因为服务器端进行正则验证，不允许上传空字符串给后端，但是可以传null值，
  ...
  const deleteHandle = (id) => { // 删除事件
    ...
    if (ids.length == 0) {
      ...
      ElMessageBox.confirm(`确定要删除选中的记录？`, '提示', {
        ...
        await post('excise/deleteExciseByIds', { ids }).then((resp) => {
...
const validatorRule = () => { //封装客户端数据验证规则
  ...
  dataRule.value = {
      title: [
        { required: false, message: "题目格式错误", trigger: "blur" },
