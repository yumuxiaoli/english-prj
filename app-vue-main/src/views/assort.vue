<template>
	<div>
		...
		>
			...
			<el-table-column prop="name" header-align="center" align="center" label="职位名称" min-width="150" />	
			<el-table-column prop="status" header-align="center" align="center" label="状态" min-width="150">
				<template #default="scope">
					 <span >{{$filters.formatOpt([{key: 'V',value: '有效'},{key: 'I',value: '无效'}], scope.row.status)}}</span>
				</template>        
			</el-table-column>	
...
import Add from './assort-add.vue';
import Update from './assort-update.vue';
  ...
  const deleteHandle = (id) => { // 删除事件
    ...
    if (ids.length == 0) {
      ...
      ElMessageBox.confirm(`确定要删除选中的记录？`, '提示', {
        ...
        await post('assort/deleteAssortByIds', { ids }).then((resp) => {
...
const validatorRule = () => { //封装客户端数据验证规则
  ...
  dataRule.value = {
      name: [
        { required: false, message: "职位名称格式错误", trigger: "blur" },
